...
# -*- coding: utf-8 -*-
"""
2026 yili tarama kapsami — %100 hedefi (ana tweet + abone + alinti + #FLOOD).
Cikti: tara_2026.json + TARAMA_2026.md (kok klasor)
"""
...
OUT_JSON = ROOT / "tara_2026.json"
OUT_MD = ROOT / "TARAMA_2026.md"
...
def _quote_incomplete(r: dict) -> bool:
    ...
    if not t:
        return True
    # Grafikli kisa alinti — X UI kirpintisi; tam metin gelmese de tamam say
...
def analyze() -> dict:
    ...
    main_2026 = [r for r in main if _is_yil(r)]
    metinli = [r for r in main_2026 if _has_text(r) and not r.get("locked")]
    bos = [r for r in main_2026 if r.get("locked") and not _has_text(r)]
    erisilemedi_ana = [
        r
        for r in main_2026
    ...
    ids_2026 = {str(r.get("tweet_id")) for r in main_2026 if r.get("tweet_id")}
    # 2026 tweetlerinden alintilanan gecmis tweetler (+ flood icindekiler)
    quotes_2026_scope = [
        q
        for q in quotes
        if str(q.get("quoted_by") or "") in ids_2026
        or _is_yil(q)
    ]
    quotes_ok = [q for q in quotes_2026_scope if not _quote_incomplete(q)]
    quotes_bad = [q for q in quotes_2026_scope if _quote_incomplete(q)]
    ...
    alinti_pct = _pct(len(quotes_ok), len(quotes_2026_scope))
    flood_pct = _pct(len(flood_roots) - len(flood_eksik), len(flood_roots))

    # Genel: en zayif halka (hepsi %100 olmali)
    parcalar = [ana_pct]
    if quotes_2026_scope:
...
def render_md(s: dict) -> str:
    ...
    tam = "EVET — 2026 %100" if s["tamam"] else "HAYIR — tarama devam"
    aylik = "\n".join(
        f"| {mo} | {v['metinli']} | {v['bos']} | "
        f"{_pct(v['metinli'], v['metinli'] + v['bos'])}% |"
        for mo, v in s.get("aylik", {}).items()
    )
    return f"""# TARAMA 2026 (@ekonomikocu)
...
| Alan | Tam | Eksik | % |
|------|-----|-------|---|
| Ana tweet (2026) | {a['metinli']} | {a['bos_kilitli']} (+{a.get('erisilemedi', 0)} erisilemedi) | **{a['yuzde']}%** |
| Alinti (2026 kapsami) | {q['tam']} | {q['eksik']} | **{q['yuzde']}%** |
| #FLOOD kok (2026) | {f['kok'] - f['eksik_parcali']} | {f['eksik_parcali']} | **{f['yuzde']}%** |
...
Yeni tweet atildiginda **TARA_2026_GUNCEL.bat** calistir (veya gunluk otomasyon).
Eksik kalmamali: ana metin, abone, alinti, flood.

---
*Otomatik: kapsam_2026.py*
"""
