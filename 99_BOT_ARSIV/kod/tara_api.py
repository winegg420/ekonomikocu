...
def parse_tweet(r):
    ...
    return {
        ...
        "conv": conv, "is_quote": is_quote, "quoted_id": quoted_id,
...
def main():
    ...
    for t in found.values():
        ...
        if t["media"]:
            folder = MEDYA / t["id"]
            if folder.exists():
                media_files = [f"medya/{t['id']}/{f.name}" for f in sorted(folder.glob('*.jpg'))]
            else:
                for j, u in enumerate(t["media"], 1):
                    dest = folder / f"graf_{j:02d}.jpg"
                    if download(u, dest):
                        media_files.append(f"medya/{t['id']}/{dest.name}")
        row = {
            "id": t["id"], "text": t["text"], "locked": False,
            "datetime": t["datetime"], "lang": "tr",
            "isQuote": t["is_quote"], "media": t["media"], "mediaFiles": media_files,
        }
        if t["quoted_id"]:
    ...
    print(f"\nIslenen Koc tweet: {len(raw_rows)} | YENI eklenen: {added} | toplam {len(by_id)}", flush=True)
    return 0
